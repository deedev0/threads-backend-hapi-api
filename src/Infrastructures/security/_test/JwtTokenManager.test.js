...
describe('JwtTokenManager', () => { 
  ...
  describe('verifyAccessToken function', () => { 
    ...
    it('should not throw InvariantError when access token verified', async () => {
      ...
      // Action and Assert
      await expect(jwtTokenManager.verifyAccessToken(accessToken)).resolves.not.toThrow(InvariantError);
    });
   });

   describe('getTokenFromHeader function', () => { 
    it('should throw AuthenticationError when header is undefined', async () => {
        // Arrange
        const jwtTokenManager = new JwtTokenManager(Jwt.token);
      ...
      it('should return access token correctly', async () => {
        // Arrange
        const jwtTokenManager = new JwtTokenManager(Jwt.token);
        const token = 'Bearer token';
        
        // Action and Assert
        await expect(jwtTokenManager.getTokenFromHeader(token)).resolves.not.toThrow(AuthenticationError);
        await expect(jwtTokenManager.getTokenFromHeader(token)).resolves.toEqual('token');
      });
  });
});
